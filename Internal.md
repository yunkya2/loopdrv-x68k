...
ここで問題になるのが、ループバックデバイスのアクセスによって、ディスクI/Oドライバのデータ読み書き処理の中から同じディスクI/Oドライバが呼び出されてしまうことです。
ディスクI/OドライバはI/Oバッファキャッシュを双方向リンクリストで管理しているのですが、自分自身の呼び出しによって繋ぎ変え途中のリンクリストが上書きされることで壊れてしまいます。

losetup.x ではこのような事態を防ぐため、Human68k オリジナルのディスクI/Oドライバにパッチを当ててリンクリストの更新が正常に行われるように修正しています。
FASTIO.X のようなディスクI/Oドライバはこの処理を丸ごと外部のものと差し替えてしまうため、ループバックドライバとの共存ができないわけです。

Human68k へのパッチ当ての詳細については、[diskiopatch.c](diskiopatch.c) のソースコードを参照してください。
